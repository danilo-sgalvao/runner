...
| `release.json` | `simulador.url`/`version`: hoje placeholder `…/danilo-sgalvao/runner/…/simulador.jar`, `version 0.0.0`. O artefato real é `hubsaude-simulador-<ver>.jar` no repo/owner da disciplina. **Correção de dados** (a struct `Simulador{URL,Version}` não muda). `simjar.Find` funciona com qualquer URL/nome — sem mudança de código. |
